...
export default function RegisterPage() {
  ...
  useEffect(() => {
    const checkRegistrationStatus = async () => {
      try {
        ...
        setDiscordEnabled(status.discordLoginEnabled);
        setGoogleEnabled(status.googleLoginEnabled);
      } catch {
        // If we can't check status, assume all are enabled
  ...
  const showOAuthSection = discordEnabled || googleEnabled;
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <p className="text-muted-foreground">
            Join the Hytale Online List community
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          {isCheckingStatus ? (
            ...
            <div className="rounded-md bg-amber-500/15 border border-amber-500/20 px-4 py-4 text-center">
          ...
          {showOAuthSection && (
            <>
              ...
              <div className={`grid gap-4 ${discordEnabled && googleEnabled ? 'grid-cols-2' : 'grid-cols-1'}`}>
                {discordEnabled && (
                  <Button
                    variant="outline"
